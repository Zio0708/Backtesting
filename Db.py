...
class StockDb(object):
    ...
    def create_StockDb(self):
        self.cursor.execute("CREATE TABLE kosdaq_leve(date text unique, high int, low int, now int);")
        self.cursor.execute("CREATE TABLE kosdaq_leve_day(date text unique, start int, end int,high int,low int);")
    ...
    def insert_Leve(self,day,high, low,now):
        self.cursor.execute("insert into kosdaq_leve values(?,?,?,?);",(day,high,low,now))

    def insert_Start(self,day,start):
        self.cursor.execute("insert into kosdaq_start values(?,?);",(day,start))

    def insert_Leve_Day(self,day,start,end,high,low):
        self.cursor.execute("insert into kosdaq_leve_day values(?,?,?,?,?);",(day,start, end,high,low))
    ...
    def insert_Profit(self, mean_level, limit, profit):
        self.cursor.execute("insert into optimize_profit values(?,?,?);",(mean_level,limit,profit))
    ...
    def select_Date(self, start, end):
        data = pd.read_sql("SELECT date FROM kosdaq_leve_day where date >= \'" + start + "\' and date <= \'"+end+ "\' order by date;", self.con, index_col=None)
        return data
    ...
    def select_Daily_Data(self,date):
        data = pd.read_sql("SELECT * FROM kosdaq_leve_day where date == \'" + date + "\';", self.con, index_col=None)
        return data
