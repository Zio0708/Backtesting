...
class Stock(QAxWidget):
    ...
    def req_day_data(self):
        self.set_input_value("종목코드","233740")
        self.set_input_value("기준일자", "20200722")
        self.set_input_value("수정주가구분",0)
    ...
    def _opt10080(self,rqname,trcode):
        ...
        for i in range(data_cnt):
            ...
            now = self._comm_get_data(trcode, "", rqname, i, "현재가")
            if(high[0] == '-'):
                high = high[1:]
            if(low[0] == '-'):
                low = low[1:]
            if(now[0] == '-'):
                now = now[1:]
            self.db.insert_Leve(day,int(high),int(low),int(now))
    ...
    def _opt10081(self,rqname, trcode):
        for i in range(150):
            ...
            if (low[0] == '-'):
                low = low[1:]
            self.db.insert_Leve_Day(day,start,end,high,low)
        self.db.commit()
